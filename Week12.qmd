...
Before getting into the details, we need to keep in mind the overall workflow of a corpus study.

```{r mermaid-example, echo=FALSE, fig.align='center'}
    DiagrammeR::mermaid("
    graph TD
        A[Ask a research question] --> B{Do I need corpus/large data} ;
        B -- No --> C[Maybe a corpus study is not appropriate!];
        C -- Yes --> D[Proceed to the corpus study!];
        D --> E{Select the right corpus};
        E --> F{Select the appropriate analysis method};
